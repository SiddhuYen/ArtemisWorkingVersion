...
def _run_turn(
    ...
    while True:
        ...
        # `web_search_20260209` does its filtering INSIDE a code-execution
        # container that the API provisions for the request. Any continuation of
        # that turn has to name the container it is continuing, or the server
        # cannot match the tool results to the sandbox that is waiting on them:
        #
        #   400 container_id is required when there are pending tool uses
        #       generated by code execution with tools
        #
        # Only turns that call a client tool mid-search hit this, which is why
        # it appeared with the network tool and not before.
        container_id = getattr(getattr(message, "container", None), "id", None)
        if container_id:
