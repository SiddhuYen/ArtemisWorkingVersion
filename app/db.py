...
def _board_tables():
    """Every table this app persists.

    Named for boards because that is all it held originally; it is now the whole
    schema. Anything added to models.py must be listed here or it silently never
    gets created — create_all is given an explicit table list, not the metadata.
    """
    from . import models
